...
export default async (_root: any, args: { id: string }, context: Context) => {
  const user = await context.db.User.findOne({ where: { id: context.session.itemId } });
  ...
  await prisma.$transaction([
    prisma.user.update({ where: { id: context.session.itemId }, data: { remainingVotes: user.remainingVotes - 1 } }),
    prisma.project.update({ where: { id: args.id }, data: { votes: newVotes } })
  ])
